...
def test_q1_bondInit():
    ...
    assert b1.face == 2e6
    assert b1.ytm == 0.03
    assert b1.coupon_rate == 0.04
    assert b1.m == 10
    assert b1.pmt_per_year == 1
...
def test_q2_convertibleBondInit():
    ...
    assert cb1.face == 2e6
    assert cb1.ytm == 0.03
    assert cb1.coupon_rate == 0.04
    assert cb1.m == 10
    assert cb1.pmt_per_year == 1
...
def test_q4_conversion_value():
    cb1 = python_hw5.ConvertibleBond(2e6, 0.03, 0.04, 10, 1, 0.5)
    assert cb1.get_conversion_value(100) == 100000


def test_q4_conversion_price():
    cb1 = python_hw5.ConvertibleBond(2e6, 0.03, 0.04, 10, 1, 0.5)
    assert cb1.get_conversion_price() == 10e6
